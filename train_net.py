...
def train_network(X, y, test, configuration, verbose, submission, validation, save):
    ...
    if validation:
        X, X_validation, y, y_validation = train_test_split(X, y, test_size=test_size,
    ...
    log_file = open(os.path.join(configuration['path'], "log"), 'w')
    ...
    for fold_n, (train_index, test_index) in iterator:
        ...
        inst_total_str = 'INST TOTAL ROC AUC {:.10f}'.format(roc_auc_score(y_original, total_prediction))
    ...
    if save:
        ...
        sub.to_csv(os.path.join(configuration['path'], "predictions.csv"), index=False)

    if submission:
        print('Submission')
        sub = pd.DataFrame({"ID_code": test_df.ID_code.values})
        sub["target"] = history_to_predictions_mean(test_predictions_history)
        sub.to_csv(os.path.join(configuration['path'], "submission.csv"), index=False)
...
if __name__ == "__main__":
    ...
    train_network(X, y, test, configuration, verbose, submission, validation, save)