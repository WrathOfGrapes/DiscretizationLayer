...
experiment_utils.create_experiment_folder(args.name)
...
def train_model(X_train, y_train, X_valid, y_valid, fold_number):
    ...
    callbacks = callbacks = [ReduceLROnPlateau(monitor='val_auroc', factor=0.5, patience=5, min_lr=1e-6, verbose=1, mode='max'),
                             ModelCheckpoint('model_' + str(fold_number), monitor='val_auroc', verbose=1, save_best_only=True, mode='max'),
    ...
    model.load_weights('model_' + str(fold_number))
...
sub.to_csv("submission_net.csv", index=False)


with open('./results.tsv', 'w+') as f:
    f.write('%s\t%f' % (args.name, roc_auc_score(y_validation, history_to_predictions_mean(train_predictions_history))))
